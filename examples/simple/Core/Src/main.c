...
size_t cb_time(dg2_user_data data)
{
    return HAL_GetTick();
}
...
void cb_yield(dg2_user_data user_data, size_t timeout_in)
{
    ...
    uint8_t buff[DG2_DISP_RX_BUFF_CAPACITY];
    HAL_UART_Receive(&huart1, buff, DG2_DISP_RX_BUFF_CAPACITY, timeout_in);
    size_t bytes_received = DG2_DISP_RX_BUFF_CAPACITY - huart1.RxXferCount;

    /* Write the packet into the circular buffer */
    dg2_circ_buff_write(&disp->rx_circ_buff, buff, bytes_received);
...
int main(void)
{
  ...
  while (1)
  {
      ...
      int16_t newPage = currentPage == 1 ? 2 : 1;
      int16_t data[2] = { 0x5A01, newPage };

      if (dg2_disp_write_vps(&disp, PIC_Set, data, DG2_ARRAY_SIZE(data)) != DG2_OK) {
          printf("Failed setting new page\n");
      }
